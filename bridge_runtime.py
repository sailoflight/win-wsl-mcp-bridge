...
MAX_SHARED_QUEUED_REQUESTS = 256
MAX_SHARED_CLIENT_QUEUED_REQUESTS = 16
MAX_SHARED_QUEUED_BYTES = 64 * 1024 * 1024
        ...
        *,
        ...
        try:
            ...
            if projection_changed:
                ...
                from installer.projection import ProjectionDatabase
...
class SharedRequestQueue:
    """Bounded per-connection FIFO lanes with round-robin dispatch."""

    def __init__(
        self,
        *,
        max_requests: int = MAX_SHARED_QUEUED_REQUESTS,
        max_client_requests: int = MAX_SHARED_CLIENT_QUEUED_REQUESTS,
        max_bytes: int = MAX_SHARED_QUEUED_BYTES,
        max_client_bytes: int = MAX_SHARED_CLIENT_QUEUED_BYTES,
    ):
        self.max_requests = max_requests
        self.max_client_requests = max_client_requests
        self.max_bytes = max_bytes
        self.max_client_bytes = max_client_bytes
        self.lanes: OrderedDict[str | None, dict[str, tuple[RoutedRequest, int]]] = OrderedDict()
        self.owners: dict[str, str | None] = {}
        self.lane_bytes: dict[str | None, int] = {}
        self.queued_bytes = 0
    ...
    def put_nowait(self, pending: RoutedRequest, size: int) -> str | None:
        owner = None if pending.is_initialize else pending.client_id
        lane = self.lanes.get(owner, {})
        lane_bytes = self.lane_bytes.get(owner, 0)
        if size > MAX_SHARED_JSONRPC_BYTES:
            return "request_size_limit"
        if owner is None:
            if len(lane) >= MAX_SHARED_INTERNAL_QUEUED_REQUESTS:
                return "internal_request_limit"
            if lane_bytes + size > MAX_SHARED_JSONRPC_BYTES:
                return "internal_byte_limit"
        else:
            if len(lane) >= self.max_client_requests:
                return "client_request_limit"
            if lane_bytes + size > self.max_client_bytes:
                return "client_byte_limit"
            if self.qsize() >= self.max_requests:
                return "backend_request_limit"
            if self.queued_bytes + size > self.max_bytes:
                return "backend_byte_limit"
        self.lanes.setdefault(owner, {})[pending.backend_id] = (pending, size)
        self.owners[pending.backend_id] = owner
        self.lane_bytes[owner] = lane_bytes + size
        self.queued_bytes += size
        self.ready.set()
        return None

    def discard(self, pending: RoutedRequest) -> None:
        if pending.backend_id not in self.owners:
            return
        owner = self.owners.pop(pending.backend_id)
        lane = self.lanes[owner]
        _pending, size = lane.pop(pending.backend_id)
        self.queued_bytes -= size
        self.lane_bytes[owner] -= size
        if not lane:
            del self.lanes[owner]
            del self.lane_bytes[owner]
        if not self.lanes:
            self.ready.clear()

    async def get(self) -> RoutedRequest:
        while not self.lanes:
            await self.ready.wait()
        owner = None if None in self.lanes else next(iter(self.lanes))
        pending, _size = next(iter(self.lanes[owner].values()))
        self.discard(pending)
        if owner in self.lanes:
            self.lanes.move_to_end(owner)
        return pending

    def clear(self) -> None:
        self.lanes.clear()
        self.owners.clear()
        self.lane_bytes.clear()
        self.queued_bytes = 0
        self.ready.clear()
...
class SharedBackend:
    ...
    def __init__(self, node: "BridgeNode", target: str, entry: dict[str, Any]):
        ...
        self.request_queue = SharedRequestQueue()
    ...
    async def _cancel_client_requests(self, client_id: str) -> bool:
        ...
        if self.state != "running":
            for (owner, request_id), pending in list(self.pending_by_client_id.items()):
                if owner != client_id:
                    continue
                self.request_queue.discard(pending)
    ...
    async def _enqueue_request(
        ...
        *,
        ...
        limit = self.request_queue.put_nowait(pending, len(_json_bytes(forwarded)) + 1)
        if limit is not None:
            if client_id is None or is_initialize:
                raise BridgeError(f"shared backend internal request rejected: {limit}")
            await self._send_client_message(
                client_id,
                {
                    "jsonrpc": "2.0",
                    ...
                    "error": {
                        "code": -32000,
                        "message": "Shared backend request was not queued or executed.",
                        "data": {
                            "reason": "shared_backend_busy",
                            "limit": limit,
                            "retryable": limit != "request_size_limit",
        ...
        if progress_token is not None:
            self.progress_tokens[progress_token[0]] = (client_id, progress_token[1])
    ...
    async def _apply_lease_policy(
        ...
        *,
        ...
        if not acquires and not is_release:
            return False
        if self.lease_owner != client_id and (self.lease_owner is not None or is_release):
            await self._send_tool_policy_error(
                client_id,
                message.get("id"),
                "client_lease_busy",
        ...
        if pending is not None and self.lease_owner is None and not is_release:
...
def build_parser(
    default_side: str, default_local_port: int, default_link_mode: str,
    *, include_installer: bool = True,
    ...
    if include_installer:
        from installer.cli import add_projection_parser
        add_projection_parser(subparsers, default_side=default_side)
    else:
        subparsers.add_parser(
            "projection", help="enroll agent environments and project peer MCP registrations",
        )
...
def component_main(default_side: str, default_local_port: int, default_link_mode: str) -> int:
    if os.name != "nt":
        os.umask(0o077)
    parser = build_parser(
        default_side, default_local_port, default_link_mode,
        include_installer=sys.argv[1:2] == ["projection"],
    )
    ...
    if args.command == "projection":
        from installer.cli import _projection_cli_main
